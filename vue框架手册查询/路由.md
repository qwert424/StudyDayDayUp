...
2. history：路由从浏览器地址栏的`location.pathname`中获取路径，改变路径使用的 H5 的`history api` 就是 history.pushStatus(null,null,'/blog') 不刷新。该模式可以让地址栏最友好，但是需要浏览器支持`history api`
