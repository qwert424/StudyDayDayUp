...
// 模拟拦截单个博客文章
Mock.mock(/^\/api\/blog\/[^/]+/, 'get', function (options) {
  const title = qs.parse(options.url).title;
  const id = qs.parse(options.url).id
  const categoryId = qs.parse(options.url)["category[id]"]
  const categoryName = qs.parse(options.url)["category[name]"];
  const thumb = qs.parse(options.url).thumb;
  const createDate = qs.parse(options.url).createDate;
  const commentNumber = qs.parse(options.url).commentNumber;
  const scanNumber = qs.parse(options.url).scanNumber;
  const description = qs.parse(options.url).description;
  return Mock.mock({
    code: 0,
    msg: "",
    data: {
      ...
      scanNumber: scanNumber, //#浏览次数
      commentNumber: commentNumber, //#评论数
      description: description,
      createDate: createDate, //#时间戳，创建日期
      toc: [
